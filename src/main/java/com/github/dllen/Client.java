...
public class Client {
    ...
    public Client(Configuration conf) throws Exception {
        this(Constants.MASTER_MAIN_CLASS, conf);
    }
    ...
    public boolean run() throws IOException, YarnException {
        ...
        if (!log4jPropFile.isEmpty()) {
            addToLocalResources(fs, log4jPropFile, LOG_4_J_PATH, appId.toString(), localResources, null);
        }

        if (shellArgs.length > 0) {
            addToLocalResources(fs, null, SHELL_ARGS_PATH, appId.toString(), localResources, StringUtils.join(shellArgs, " "));
        }

        if (javaOpts.length > 0) {
            addToLocalResources(fs, null, JAVA_OPTS_PATH, appId.toString(), localResources, StringUtils.join(javaOpts, " "));
        }
